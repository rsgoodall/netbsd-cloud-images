...
file="final.raw"
mkdir -p ${MNT}
dd if=/dev/zero of=${file} bs=4096 count=450000 progress=62000
vnconfig ${VND} ${file}
gpt create ${VND}
...
chroot $MNT sh -c '. /etc/profile; pkg_add py312-pip'
chroot $MNT sh -c '. /etc/profile; pkg_add mozilla-rootcerts'
chroot $MNT sh -c '. /etc/profile; mozilla-rootcerts install'
...
chroot $MNT sh -c '. /etc/profile; pip3.12 install jsonpointer jsonpatch'
